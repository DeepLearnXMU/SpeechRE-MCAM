...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        # catalog shared parameters  是吧共享参数需要共享的参数名字
        ...
        if cfg.distributed_training.ddp_backend != "fully_sharded":
            if cfg.common.fp16:
                ...
                # self._model.bart.model = self._model.bart.model.half()
            elif cfg.common.bf16:
                self._criterion = self._criterion.to(dtype=torch.bfloat16)
                self._model = self._model.to(dtype=torch.bfloat16)
                # self._model.bart.model = self._model.bart.model.to(dtype=torch.bfloat16)
        if (
            ...
            # self._model.bart.model =  self._model.bart.model.to(device=self.device)
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        self.model.bart.model.encoder.train()
        self.model.bart.model.decoder.train()
        self.zero_grad()
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                # take an optimization step  模型的梯度下降
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            self.model.eval()
            self.model.bart.model.encoder.eval()
            self.model.bart.model.decoder.eval()
            self.criterion.eval()
        ...
        return logging_output, logging_outputs[0]

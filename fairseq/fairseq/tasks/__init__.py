...
def setup_task(cfg: FairseqDataclass, **kwargs):
    ...
    if isinstance(task_name, str):
        # legacy tasks
        task = TASK_REGISTRY[task_name]  #取出默认任务
        ...
        if task_name and task_name in TASK_DATACLASS_REGISTRY:
            dc = TASK_DATACLASS_REGISTRY[task_name]  #获取自定义任务的task的参数类
            cfg = merge_with_parent(dc(), cfg)  #将任务的配置类的参数合并到cfg中 将dataclass类中参数合并到cfg
            task = TASK_REGISTRY[task_name]  #取出任务类的

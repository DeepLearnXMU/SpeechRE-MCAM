...
class BARTHubInterface(GeneratorHubInterface):
    ...
    def __init__(self, cfg, task, model):
        ...
        # self.special_tokens = ['<mask>','<triplet>', '<subj>', '<obj>', '<peop>', '</peop>', '<org>', '</org>', '<other>',
        #                     '</other>', '<loc>', '</loc>']
        self.special_tokens = ['<mask>', '<triplet>', '<subj>', '<obj>', '<entity>', '</entity>', '<blank>']
    ...
    def encode(
        ...
        for i, w in enumerate(token_list):
            ...
            if(w in self.special_tokens):
                ...
                tokens += (pre + self.bpe.encode((pre + w)))
        ...
        # self.task.source_dictionary.indices   model.bart.task.source_dictionary.indices
    ...
    def decode(self, tokens: torch.LongTensor):
        ...
        sentences_temp = []
        for s in sentences:
            sentence_str = self.task.source_dictionary.string(s)
            tokens_list = sentence_str.split()
            sentence = ""
    ...
    def generate(
        ...
        **kwargs
        ...
        res = []
        # tokenized_sentences = torch.stack(tokenized_sentences, dim=0)
